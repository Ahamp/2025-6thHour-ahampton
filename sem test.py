#Name:Aaden hampton
#Class: 6th Hour
#Assignment: Semester Project 1

import random
import time
print("hello world")
#Due to weird time travelling circumstances beyond explanation, you find yourself in 2018 or so
#working for Larian Studios. Currently, they are working on the early prototypes of the hype
#upcoming game "Baldur's Gate 3". BG3 is a game that uses the Dungeons & Dragons 5th edition rules
#as its framework for gameplay. You have been given a basic dictionary of some of the main hero
#characters and some of the enemies they may face, and are tasked with making an early prototype
#of one of the party members fighting against an enemy until one of them hits zero HP (dies).

partyDict = {
    "LaeZel" : {
        "HO" : 48,
        "Init" : 1,
        "AC" : 17,
        "AtkMod": 6,
        "Damage" : random.randint(1,6) + random.randint(1,6) + 3
    },
    "Shadowheart" : {
        "HP" : 40,
        "Init" : 1,
        "AC" : 18,
        "AtkMod": 4,
        "Damage" : random.randint(1,6) + 3,
    },
    "Gale" : {
        "HP" : 32,
        "Init" : 1,
        "AC" : 14,
        "AtkMod": 6,
        "Damage" : random.randint(1,10) + random.randint(1,10),
    },
    "Astarion" : {
        "HP" : 40,
        "Init" : 3,
        "AC" : 14,
        "AtkMod": 5,
        "Damage" : random.randint(1,8) + random.randint(1,6) + 4,
    }
}

enemyDict = {
    "Goblin" : {
        "HP" : 7,
        "Init" : 0,
        "AC" : 12,
        "AtkMod": 4,
        "Damage" : random.randint(1,6) + 2
    },
    "Orc": {
        "HP" : 15,
        "Init": 1,
        "AC" : 13,
        "AtkMod": 5,
        "Damage" : random.randint(1,12) + 3
    },
    "Troll": {
        "HP" : 84,
        "Init": 1,
        "AC" : 15,
        "AtkMod": 7,
        "Damage" : random.randint(1,6) + random.randint(1,6) + 4
    },
    "Mindflayer": {
        "HP" : 71,
        "Init": 1,
        "AC" : 15,
        "AtkMod": 7,
        "Damage" : random.randint(1,10) + random.randint(1,10) + 4
    },
    "Dragon": {
        "HP" : 127,
        "Init": 2,
        "AC" : 18,
        "AtkMod": 7,
        "Damage" : random.randint(1,10) + random.randint(1,10) + random.randint(1,8) + 4
    },
}

#Combat consists of these steps:

#1. Rolling for 'initiative' to see who goes first. This is determined by rolling a
#20-sided die (d20) and adding their initiative modifier (If the roll is the same,
#assume the hero goes first).
playermoves = False
enemymoves = False
random.randint(1,20 )
player_init = random.randint(1,20 ) + partyDict["LaeZel"]["Init"]
print("the player rolled a", player_init)
enemy_init = random.randint(1,20 ) + enemyDict["Orc"]["Init"]
print("the enemy rolled a", enemy_init)
if player_init > enemy_init:
    print ("The Hero goes first!")
    varplayermoves = True
elif enemy_init <= player_init:
     print ("The Enemy goes first!")
     enemymoves = True
elif player_init == enemy_init:
     print ("It's a tie! The Hero goes first!")
     playermoves = True


#2. Rolling to attack. This is determined by rolling a 20-sided die (d20) and adding their
#attack modifier. The attack hits if it matches or is higher than the target's Armor Class (AC).
#If the d20 rolled to attack is an unmodified ("natural") 20, the attack automatically hits and
#the character deals double damage. If the d20 rolled to attack is an unmodified ("natural") 1,
#the attack automatically misses
while partyDict["LaeZel"]["HO"] > 0 or enemyDict["Orc"]["HO"]>0:

    varplayerattack =  random.randint(1,20)+ partyDict["LaeZel"]["AtkMod"]
    playernaturalattack = random.randint(1,20)
    if playermoves:
        if playernaturalattack == 20:
            enemyDict["Orc"]["HP"] -= partyDict["LaeZel"]["Damage"] * 2
            print("player rolled a natural 20")
            playermoves = False
            enemymoves = True
        if playernaturalattack == 1:
            print("player missed")
        if varplayerattack >=10:
            print("the player rolled a", varplayerattack)
            playermoves = False
            enemymoves = True
            enemyDict["Orc"]["HP"] -= partyDict["LaeZel"]["Damage"]

        if varplayerattack <10:
            playermoves = False
            enemymoves = True
            print("missed")
        else:
            print("Orc gets miss")
            enemyDict["Orc"]["HP"] -= varplayerattack

        if playernaturalattack <= enemyDict["Orc"]["AC"]:
            playermoves = False
            enemymoves = True
            print("Cant get through armor")

    varenemyattack = random.randint(1, 20) + enemyDict["Orc"]["AtkMod"]
    enemynatural = random.randint(1,20)
    if enemymoves==True:
        if enemynatural == 20:
            partyDict["LaeZel"]["HP"] -= partyDict["Orc"]["Damage"] * 2
            print("enemy rolled a natural 20")
            enemymoves = False
            playermoves = True
        if enemynatural == 1:
            print("enemy missed")
            enemymoves = False
            playermoves = True
        if varenemyattack >= 10:
            print("the enemy rolled a", varenemyattack)
            enemymoves = False
            playermoves = True
            partyDict["LaeZel"]["HP"] -= enemyDict["Orc"]["Damage"]

        if varenemyattack < 10:
            enemymoves = False
            playermoves = True
            print("missed")
        else:
            print("LaeZel gets miss")
            partyDict["LaeZel"]["HP"] -= varenemyattack

        if enemynatural <= partyDict["LaeZel"]["AC"]:
            enemymoves = False
            playermoves = True
            print("Cant get through armor")
        if partyDict["LaeZel"]["HP"] <= 0:
            break
        elif enemyDict["Orc"]["HP"] <= 0:
            break
#3. If the attack hits, roll damage and subtract it from the target's hit points.


#4. The second in initiative rolls to attack (and rolls damage) afterwards.

#5. Repeat steps 2-5 until one of the characters is dead.
"""if enemyDict["Orc"]["AC"] < varplayerattack:
    print("Orc gets hit")
enemyDict["Orc"]["HP"] -= partyDict["LaeZel"]["Damage"]   if  enemyDict["Orc"]["AC"] < varplayerattack:
        print("Orc gets hit")
       enemyDict["Orc"]["HP"] -= partyDict["LaeZel"]["Damage"]"""